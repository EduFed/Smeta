...
const CRITICAL_LIBS = [
  'https://data.smetaraschet.ru/static/lib/supabase.js',
  'https://data.smetaraschet.ru/static/fonts.css'
];
...
self.addEventListener('fetch', (e)=>{
  ...
  // Имён у сервера два: data — рабочее, api — прежнее. Старое оставлено
  // рабочим, потому что у людей в кэше лежат файлы, запрошенные по нему,
  // и резкая смена сломала бы им офлайн-режим.
  const OUR_SERVER = (url.hostname === 'data.smetaraschet.ru' ||
                      url.hostname === 'api.smetaraschet.ru');
